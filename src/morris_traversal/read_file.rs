...
pub fn remote_duplicate(split_path:Vec<Vec<String>> ) -> HashMap<usize, Vec<String>> {
        ...
        if let Some(path) = same_path.get(&i) {
            ...
            if current_path.is_empty() {
                ...
                if let Some(last_index) = path.last() {
                    // println!("last element: {:?}", last_index);
                    same_directory.push(last_index)
                }
                // println!("same_directory: {:?}", same_directory.len())
            }else{
                same_path_file.insert(current_path, same_directory.clone());
                current_path = new_path;
